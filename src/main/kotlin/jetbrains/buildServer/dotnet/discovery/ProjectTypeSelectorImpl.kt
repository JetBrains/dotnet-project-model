...
class ProjectTypeSelectorImpl : ProjectTypeSelector {
    ...
    private fun isPublishProject(project: Project): Boolean =
        project.generatePackageOnBuild || project.references.filter {
            PublishReferencePattern.matcher(it.id).find()
        }.any()

    private companion object {
        private val PublishReferencePattern: Pattern =
            Pattern.compile("^Microsoft\\.aspnet.+$", Pattern.CASE_INSENSITIVE)
        private val TestReferencePattern: Pattern =
            Pattern.compile("^Microsoft\\.NET\\.Test\\.Sdk$", Pattern.CASE_INSENSITIVE)
    }
}