...
class DotnetRunnerDiscoveryExtension(
    ...
    override fun discoverRunnersInDirectory(
        ...
        discover(
            StreamFactoryImpl(dir.browser),
            getElements(filesAndDirs.asSequence()).map { it.fullName }).toMutableList()

    override fun postProcessDiscoveredObjects(
        settings: BuildTypeSettings,
        browser: Browser,
        discovered: MutableList<DiscoveredObject>
    ): MutableList<DiscoveredObject> =
    ...
    fun discover(streamFactory: StreamFactory, paths: Sequence<String>): Sequence<DiscoveredTarget> {
        ...
        val simpleSolutions =
            solutions.asSequence().filter { it.isSimple && !complexSolutionProjects.containsAll(it.projects) }
        return complexSolutions.asSequence().plus(simpleSolutions).flatMap { createCommands(it) }.distinct()
            .map { createTarget(it) }
    }

    private fun extractParameters(parameters: Map<String, String>): List<Parameter> =
        parameters.filter { Params.contains(it.key) && !it.value.isBlank() }.map {
            Parameter(
                it.key,
                it.value
            )
    ...
    private fun getElements(elements: Sequence<Element>, depth: Int = 3): Sequence<Element> =
        if (depth > 0)
            elements.filter { it.isLeaf && it.isContentAvailable }.plus(elements.filter { !it.isLeaf && it.children != null }.flatMap {
                getElements(
    ...
    private fun createSimpleCommand(commandType: DotnetCommandType, path: String): Command =
        Command(
            ...
            listOf(
                Parameter(DotnetConstants.PARAM_COMMAND, commandType.id),
